...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Anchor, Share2, Server, Repeat, AlertTriangle, Layers, Wallet, Landmark, CreditCard, PieChart, ShieldAlert } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">FSLZ Maestro</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Dashboard" active />
            <NavItem to="/entities" icon={<Globe size={20} />} label="Legal Entities" />
            <NavItem to="/identity" icon={<Users size={20} />} label="Identity Governance" />
            <NavItem to="/cost" icon={<Wallet size={20} />} label="Cost Insights" />
            <NavItem to="/compliance" icon={<ShieldCheck size={20} />} label="Compliance Review" />
            <NavItem to="/onboarding" icon={<Box size={20} />} label="Workload Onboarding" />
            <NavItem to="/kpi" icon={<PieChart size={20} />} label="KPI Scorecards" />
          </nav>

          <div className="pt-6 border-t border-slate-800">
            <NavItem to="/settings" icon={<Settings size={20} />} label="FSLZ Settings" />
          </div>
        </aside>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              <span>Industrial Financial Cloud</span>
              <span>/</span>
              <span className="text-white font-bold">Financial Services Landing Zone</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Cloud Architect</p>
                <p className="text-[10px] text-indigo-400 uppercase tracking-widest font-black">Institutional Access</p>
              </div>
              <div className="w-10 h-10 bg-slate-800 rounded-full border border-slate-700 flex items-center justify-center font-bold text-slate-300">FS</div>
            </div>
          </header>
